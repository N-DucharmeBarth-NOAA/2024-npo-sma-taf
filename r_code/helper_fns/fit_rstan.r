...
# You should have received a copy of the GNU General Public License along with this program.  If not, see <https://www.gnu.org/licenses/>.
    ...
    fit_rstan = function(stan.data,stan_c,run_label,exec_name,n_cores,seed,chains,n_thin,iter_keep,burnin.prop,adapt_delta,max_treedepth,
                        silent=TRUE,stan_code_dir = "./boot/software/",stan_save_dir="./model/"){
        # packages
        require(rstan)
        require(parallelly)
        require(data.table)
        ...
        # define run characteristics
            ...
            run_retro = as.numeric(strsplit(run_label,"_")[[1]][3])
        ...
        # define the inits
            set.seed(seed)
            stan.inits = replicate(chains,stan_inits_func(version=exec_name,Tm1 = (stan.data$T-1)),simplify=FALSE)
        ...
        # summarize fit
            ...
            # priors
                ...
                possible_priors = c(paste0("PriorMean_",c("logK","logr","logx0","logshape","logsigmap","logsigmao")),
                                    paste0("PriorSD_",c("logK","logr","logx0","logshape","logsigmap","logsigmao")),
        ...
        # write-out summaries
            dir_output = paste0(stan_save_dir,run_label,"/")
            ...
            fwrite(fit_summary,paste0(dir_output,"fit_summary.csv"))
            fwrite(hmc_samples,paste0(dir_output,"hmc_samples.csv"))
            fwrite(settings_dt,paste0(dir_output,"settings.csv"))
            fwrite(neff_dt,paste0(dir_output,"neff.csv"))
            fwrite(rhat_dt,paste0(dir_output,"rhat.csv"))
            fwrite(stan_data,paste0(dir_output,"stan_data.csv"))
            fwrite(stan_inits,paste0(dir_output,"stan_inits.csv"))
