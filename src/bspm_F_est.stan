...
transformed parameters {
    ...
    // population dynamics
        ...
        for(t in 2:T) {
            if(x[t-1]<=dmsy){
                x[t] = ((x[t-1] + r * x[t-1] * (1 - x[t-1]/h)))*epsp[t-1]*(exp(-F[t-1]));
                removals[t-1] = ((x[t-1] + r * x[t-1] * (1 - x[t-1]/h)))*epsp[t-1]*(1-exp(-F[t-1]))*exp(logK);
            } 
            if(x[t-1]> dmsy){
                x[t] = ((x[t-1] + g * m * x[t-1] * (1 - pow(x[t-1],(n-1)))))*epsp[t-1]*(exp(-F[t-1]));
                removals[t-1] = ((x[t-1] + g * m * x[t-1] * (1 - pow(x[t-1],(n-1)))))*epsp[t-1]*(1-exp(-F[t-1]))*exp(logK);
            } 
        }
