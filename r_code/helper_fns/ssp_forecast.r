...
ssp_forecast = function(ssp_summary,samples_dt,stan_data,settings,sub_sample_prop=0.1,forecast_years=5,forecast_type=c("Catch","U","MSY","Umsy")[1],avg_years=3,scalar=1,resample_raw_epsp="TRUE",exclude_last=0){
      ...
      if(!("removals" %in% unique(samples_dt$name))){
            ...
            removals_mat = matrix(NA,nrow=length(iter_sampled),ncol=T+forecast_years)
      } else {
      ...
      for(i in 1:nrow(forecast_x)){
            if(!("removals" %in% unique(samples_dt$name))){
                  removals_mat[i,1:T] = removals
                  C = min(c(exp(log(removals[T]) - logK[i]),x[i,T]))
            } else {
      ...
      # determine forecast catch levels
      if(forecast_type == "Catch"){
            if(!("removals" %in% unique(samples_dt$name))){
                  catch_vec = rep(mean(removals[((length(removals)-(avg_years-1)):(length(removals)-exclude_last))])*scalar,length(iter_sampled))
            } else {
            ...
            if(!("removals" %in% unique(samples_dt$name))){
                  ...
                  for(i in 1:length(iter_sampled)){
                        u_vec[i] = mean(removals[((length(removals)-(avg_years-1)):(length(removals)-exclude_last))]/(x[i,][((length(x[i,])-(avg_years-1)):(length(x[i,])-exclude_last))]*exp(logK[i])))*scalar
                  }
            } else {
                  u_vec = rep(NA,length(iter_sampled))
                  for(i in 1:length(iter_sampled)){
      ...
      # repackage, add new rows to raw_epsp (29+ ) & x (30 +)
            ...
            removals_dt = as.data.table(removals_mat) %>%
                    ...
                    .[order(chain,iter)]
            if(!("removals" %in% unique(samples_dt$name))){
                  out = samples_dt[!(name%in%c("raw_epsp","x"))] %>%
                        rbind(.,new_x_dt) %>%
